...
(defun first-run-agent ()
  (let ((info-string "~d => This is the ~a background color scheme.")
	(*color-scheme* *color-scheme-darkbg*)
	(config-args nil))
    (info (format nil info-string 1 "dark"))
    (setf *color-scheme* *color-scheme-lightbg*)
    (info (format nil info-string 2 "light")))
  (let ((result (getline (with-term-colors/id :pkg-outofdate
			   (format nil "Please select your preferred color scheme => ")))))
    (cond ((string= result "1")
	   (push "(setf *color-scheme* *color-scheme-darkbg*)" config-args))
	  ((string= result "2")
	   (push "(setf *color-scheme* *color-scheme-lightbg*)" config-args))))
  (let ((result (getline (with-term-colors/id :pkg-outofdate
			   (format nil "Please select paktahn's directory: (default ~/.paktahn)~%=> ")))))
    (when (char= (aref result 0) #\~)
      (setf result (format nil "~a~a" (homedir) (subseq result 2))))
    (push (format nil "(setf *paktahn-dir* ~s)" result) config-args)
  (write-user-config config-args)))

(defun write-user-config (&rest args)
  (with-open-file (out (config-file "config.lisp") :direction :output)
    (format out "(in-package :pak)~%~%")
    (loop for option in args do
      (format out "~a~%" option))
    (format out "~%;; This config file generated by paktahn~%")))
